...
def test_rbm(learning_rate: float=0.1, training_epochs: int=15,
             dataset: str='mnist.pkl.gz', batch_size: int=20,
             n_chains: int=20, n_samples: int=10, output_folder: str='rbm_plots',
             n_hidden: int=500):
    """
    ...
    rbm = RestrictedBoltzmannMachine(
        input=x, n_visible=28 * 28, n_hidden=n_hidden,
    ...
    for epoch in range(training_epochs):
        ...
        image = Image.fromarray(
            tile_raster_images(
                X=rbm.weights.get_value(borrow=True).T,
                img_shape=(28, 28),
                tile_shape=(10, 10),
                tile_spacing=(1, 1)
            )
